...
void findStartMD5(char* extension,char* startDirname,char* minByte,char* minByteState,char* maxByte,char* maxByteState);
void getMD5hash(char* pathName,unsigned char* md);
...
int main(int argc,char* argv[]){
	...
	findStartMD5(argv[1],argv[6],argv[2],argv[4],argv[3],argv[5]);
									...
									if(isInfinite){
										if(buf.st_size>=minByteNum){
											if(strcmp(extension,"*")==0){
												...
												if(md5!=NULL){
													enroot(root,sq,nextPath,buf.st_size,md5);
												}
											}
											else{ //지정한 확장자가 있는 경우.
												size_t exLength=strlen(extension);
...
void getMD5hash(char* pathName,unsigned char* md){
	...
	if((fd=open(pathName,O_RDONLY))<0){
		fprintf(stderr,"open error..can't find md5\n");
		md=NULL;
		return;
	}
 	int readSize=0;
	static unsigned char buf[BUFSIZE];
	MD5_CTX c; //해쉬값을 초기화 해갈것임.
	
	MD5_Init(&c); //해쉬값 계산 전 초기화
	while((readSize=read(fd,buf,BUFSIZE))>0){
		MD5_Update(&c,buf,(unsigned long)readSize);
	}
	MD5_Final(&(md[0]),&c); //최종적인 해쉬값을 저장. 
	close(fd);
	return;
}
