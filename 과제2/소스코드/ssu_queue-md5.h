...
typedef struct Queue{
	...
	unsigned char md[MD5_DIGEST_LENGTH]; //해당 큐가 담당하는 해쉬값.
...
void initqueue(Queue* q,off_t fSize,unsigned char* md5){
	q->front=NULL;
	q->rear=NULL;
	q->count=0; //큐에 있는 노드의 수.
	q->size=fSize; //해당 큐에 들어갈 파일들의 크기
	memcpy(q->md, md5, MD5_DIGEST_LENGTH); //해쉬값 복사. 해당 큐가 맡을 해쉬의 정보.
	q->next=NULL;
}
...
void enroot(Root* r,SizeQueue* sq,char* path,off_t fSize,unsigned char* md5){
	int isFindInSQ=0; 
...
void swqpQdata(Queue* q1,Queue* q2){
	...
	unsigned char TMPmd[MD5_DIGEST_LENGTH];
	memcpy((unsigned char*)TMPmd,(unsigned char*)(q1->md),(size_t)MD5_DIGEST_LENGTH);

	q1->front=q2->front;
	q1->rear=q2->rear;
	q1->count=q2->count;
	q1->size=q2->size;
	memcpy((unsigned char*)(q1->md),(unsigned char*)(q2->md),(size_t)MD5_DIGEST_LENGTH);

	q2->front=TMPfront;
	q2->rear=TMPrear;
	q2->count=TMPcount;
	q2->size=TMPsize;
	memcpy((unsigned char*)(q2->md),(unsigned char*)(TMPmd),(size_t)MD5_DIGEST_LENGTH);
}
...
void enqueueSize(SizeQueue* s,off_t fSize){
	if(s->count==0){
		...
		for(int i=0; i < s->count ; i++){
			...
			if(same){
				findQueue=1; //큐를 찾았으면 1로
				indexN->count='2'; //해당 크기를 가진 파일이 2개 이상임을 표시.
				break;
			}
